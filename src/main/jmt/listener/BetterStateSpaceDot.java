 ...
 */
public class BetterStateSpaceDot extends ListenerAdapter {
  ...
  public BetterStateSpaceDot(Config conf, JPF jpf) {
   ...
   *
   * @param  state Relevent state to form the label for
   * @return       String representing what the state should be labelled
   */
  ...
  private void addNode(StateInformation state) throws IOException {
    if (state.is_new) {
      if (format==GDF_FORMAT) {
        graph.write("st" + state.id + ",\"" + makeStateLabel(state));
        if (state.error != null) {
  ...
  static void showUsage() {
    System.out
        .println("Usage: \"java [<vm-options>] gov.nasa.jpf.tools.BetterStateSpaceDot [<graph-options>] [<jpf-options-and-args>]");
